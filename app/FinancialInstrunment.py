...
class FinancialInstrunment(): 
    ...
    def plot_returns(self, freq=None, kind='line'): 
        """
            Plot returns with specified frequency and kind (line or hist)
            freqs = ["YE", "QE", "ME", "W-FRI", "D"]
            periods = [1, 4, 12, 52, 252] for freqs 
        """
        if freq is None: 
            self.data[self.log_return_name].plot(kind=kind)
        else: 
            resampled_price = self.data[self.price_name].resample(freq).last()
            resampled_returns = np.log(resampled_price / resampled_price.shift(1))
            resampled_returns.plot(kind = kind)
        self._setup_plot("Log Return Chart")

    def mean_return(self, freq="D"): 
        '''
        '''
